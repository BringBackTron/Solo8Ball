...
class Shape {
    constructor(x, y, radius, ax, ay, m, vx=0, vy=0, color) {
        this.x = x; //x location
        this.y = y; //y location
        this.r = radius; //size of circle
        this.ax = ax; //acceleration of x
        this.ay = ay; // acceleration of y
        this.m = m; // mass
        this.vx = vx; //velocity of x
        this.vy = vy; //velocity of y
        this.fx = 0; //force on x for gravity
        this.fy = 0; //force on y for gravity
        this.color = color;
    }
...
function createGravityExample() {
    ...
    objects.push(new Shape(640, 632, 20, 0, 0, 45));
...
function createBouncingExample() {
    ...
    let rows = 5;
    let radius = 20;
    let startX = Math.round(c.offsetWidth/8);
    let startY = Math.round(c.offsetHeight/3);
    let cols = Math.round(c.offsetHeight * .01)/radius; // 20% filled with balls (by height)
    for (let i = 0; i < rows; i++) {
        for (let j = 0; j < cols; j++) {
            objects.push(new Shape(startX + j*radius, startY + i*radius, radius, 0, 0, 100))
        }
...
/*    for (let i = 0; i < rows-1; i++) {
        for (let j = 0; j < cols; j++) {
            objects.push(new Shape(startX + j*radius + 40, startY + i*radius + 5, radius, 0, 0, 100))
        }
    }

    for (let i = 0; i < rows-2; i++) {
        for (let j = 0; j < cols; j++) {
            objects.push(new Shape(startX + j*radius + 80, startY + i*radius + 10, radius, 0, 0, 100))
        }
    }

    for (let i = 0; i < rows-3; i++) {
        for (let j = 0; j < cols; j++) {
            objects.push(new Shape(startX + j*radius + 120, startY + i*radius + 15, radius, 0, 0, 100))
        }
    }

    for (let i = 0; i < rows-4; i++) {
        for (let j = 0; j < cols; j++) {
            objects.push(new Shape(startX + j*radius + 140, startY + i*radius + 25, radius, 0, 0, 100))
        }
    }

    for (let i = 0; i < rows-4; i++) {
        for (let j = 0; j < cols; j++) {
            objects.push(new Shape(startX + j*radius + c.offsetWidth/2, startY + i*radius + 25, radius, 0, 0, 100))
        }
    }*/
